...
export function RequestsAnalyticsWorkspace() {
  ...
  const barData = barQuery.data?.data as AnalyticsBarData | undefined
  const scatterData = scatterQuery.data?.data as { points: AnalyticsScatterPoint[] } | undefined
  const sankeyData = sankeyQuery.data?.data as AnalyticsSankeyData | undefined
    ...
    <div className="flex min-h-full flex-col gap-6">
        ...
        <CardContent className="space-y-4 pt-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <MultiSelect
              ...
              placeholder={t('charts.filters.models')}
              searchPlaceholder={t('charts.filters.searchModels')}
              emptyText={t('charts.filters.noModels')}
              selectedText={t('charts.filters.selected')}
              triggerVariant="filter"
              triggerLabel={t('charts.filters.models')}
            ...
            <MultiSelect
              ...
              placeholder={t('charts.filters.sites')}
              searchPlaceholder={t('charts.filters.searchSites')}
              emptyText={t('charts.filters.noSites')}
              selectedText={t('charts.filters.selected')}
              triggerVariant="filter"
              triggerLabel={t('charts.filters.sites')}
              dropdownWidthMode="content"
            />
            <MultiSelect
              ...
              placeholder={t('charts.filters.apiKeys')}
              searchPlaceholder={t('charts.filters.searchApiKeys')}
              emptyText={t('charts.filters.noApiKeys')}
              selectedText={t('charts.filters.selected')}
              triggerVariant="filter"
              triggerLabel={t('charts.filters.apiKeys')}
              dropdownWidthMode="content"
            />
            {availableCurrencies.length > 1 ? (
              <Select value={draft.currency || availableCurrencies[0]} onValueChange={(currency) => patchDraft({ currency })}>
                <SelectTrigger variant="filter" filterLabel={t('charts.filters.currency')} active={Boolean(draft.currency)}>
                  <SelectValue />
                </SelectTrigger>
      ...
      <div className="grid gap-5">
        <ChartCard
          ...
          footer={barQuery.data ? `${t('charts.totalRequests')}: ${barQuery.data.meta.total_requests.toLocaleString()} · ${t('charts.currency')}: ${barQuery.data.meta.currency}` : undefined}
        >
        ...
        <ChartCard
          ...
          footer={scatterQuery.data?.meta.too_many_points ? t('charts.scatter.tooMany') : scatterQuery.data ? `${t('charts.totalRequests')}: ${scatterQuery.data.meta.total_requests.toLocaleString()}` : undefined}
        >
        ...
        <ChartCard
          ...
          footer={sankeyData ? `${t('charts.totalTokens')}: ${sankeyData.total_tokens.toLocaleString()}` : undefined}
        >
